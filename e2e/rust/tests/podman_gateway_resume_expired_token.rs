...
//! Podman e2e coverage for gateway-owned sandbox JWT file rotation.
...
const READY_MARKER: &str = "podman-gateway-resume-expired-token-ready";
const RESUME_FILE: &str = "/sandbox/podman-gateway-resume-expired-token-state";
...
#[tokio::test]
async fn podman_gateway_restart_reissues_expired_sandbox_token_for_running_container() {
    if std::env::var("OPENSHELL_E2E_DRIVER").as_deref() != Ok("podman") {
        eprintln!("Skipping Podman expired-token resume test: e2e driver is not podman");
        return;
    }
    let Some(ttl_secs) = short_sandbox_jwt_ttl_secs() else {
        eprintln!(
            "Skipping Podman expired-token resume test: set OPENSHELL_E2E_SANDBOX_JWT_TTL_SECS to 1..=10"
        );
        return;
    };
    let Some(gateway) = ManagedGateway::from_env().expect("load managed e2e gateway metadata")
        ...
        eprintln!(
            "Skipping Podman expired-token resume test: e2e gateway is not managed by this test run"
        );
        return;
    };
    ...
    .await
    .expect("Podman sandbox should be ready before gateway stop");
    ...
    .await
    .expect("Podman sandbox should reconnect after startup reissued the token file");
