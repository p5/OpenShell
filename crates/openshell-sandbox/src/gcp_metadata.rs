...
//!
//! The emulator is intercepted in the forward-proxy path (like `policy.local`)
//! at hostname `gcp.metadata.openshell.internal`. GCP SDKs discover it via
//! the `GCE_METADATA_HOST` environment variable. The
//! `*.metadata.openshell.internal` suffix is reserved for cloud-provider
//! metadata emulators.
...
const PATH_SERVICE_ACCOUNT_DEFAULT: &str =
    ...
    match route {
        ...
        PATH_SERVICE_ACCOUNT_DEFAULT => {
            if recursive {
                ...
                (200, "text/plain", "aliases\nemail\nscopes\ntoken\n".to_string())
            }
        }
        PATH_SERVICE_ACCOUNTS => {
            (200, "text/plain", "default/\n".to_string())
        }
        "" | "/" | "/computeMetadata" | "/computeMetadata/v1" => {
            (200, "text/plain", "computeMetadata/\n".to_string())
        }
        "/computeMetadata/v1/instance" => {
            (200, "text/plain", "service-accounts/\n".to_string())
        }
        _ => {
            emit_metadata_event(
                ActivityId::Refuse,
...
fn handle_env(ctx: &MetadataContext, env_key: &str) -> MetadataResponse {
    ...
    match resolver.resolve_placeholder(&placeholder) {
        Some(value) => (200, "text/plain", value.to_string()),
        None => {
            emit_metadata_event(
                ActivityId::Fail,
                SeverityId::Low,
...
mod tests {
    ...
    fn token_expires_in_computed_from_credential_expiry() {
        let now_ms = secrets::current_time_ms();
        ...
        assert!(expires_in > 1700 && expires_in <= 1800, "expires_in={expires_in}");
    }
    ...
    fn project_id_served_as_plain_text() {
        ...
        let (status, ct, body) =
            route_request(&ctx, "GET", PATH_PROJECT_ID, &flavor_headers());
        assert_eq!(status, 200);
