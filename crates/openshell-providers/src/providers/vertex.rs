...
impl ProviderPlugin for VertexPlugin {
    ...
    fn discover_existing(&self) -> Result<Option<DiscoveredProvider>, ProviderError> {
        let mut discovered =
            discover_with_spec(&SPEC, &RealDiscoveryContext)?.unwrap_or_default();
    ...
    fn inject_env(&self, provider: &Provider, env: &mut HashMap<String, String>) {
        let project_id_key = gcp::config_key("google-vertex-ai", "project_id")
            .expect("vertex project_id key defined");
        let region_key =
            gcp::config_key("google-vertex-ai", "region").expect("vertex region key defined");

        if let Some(project) =
            provider.config.get(project_id_key).filter(|v| !v.trim().is_empty())
        {
            ...
            env.entry(gcp::ANTHROPIC_VERTEX_PROJECT_ID_ENV_VAR.to_string())
                .or_insert_with(|| trimmed.clone());
        }

        if let Some(region) =
            provider.config.get(region_key).filter(|v| !v.trim().is_empty())
        {
            ...
            env.entry(gcp::VERTEX_LOCATION_ENV_VAR.to_string())
                .or_insert_with(|| trimmed.clone());
        }

        env.entry(gcp::VERTEX_USE_ENV_VAR.to_string())
            .or_insert_with(|| "1".to_string());
        env.entry(gcp::GOOSE_PROVIDER_ENV_VAR.to_string())
...
mod tests {
    ...
    fn injects_project_id_and_anthropic_alias() {
        ...
        VertexPlugin.inject_env(&provider, &mut env);
    ...
    fn injects_region_and_vertex_location() {
        ...
        assert_eq!(env.get("CLOUD_ML_REGION").map(String::as_str), Some("us-east4"));
        assert_eq!(env.get("GCP_LOCATION").map(String::as_str), Some("us-east4"));
        assert_eq!(env.get("VERTEX_LOCATION").map(String::as_str), Some("us-east4"));
    }
    ...
    fn injects_inference_flags() {
        ...
        assert_eq!(env.get("CLAUDE_CODE_USE_VERTEX").map(String::as_str), Some("1"));
        assert_eq!(
